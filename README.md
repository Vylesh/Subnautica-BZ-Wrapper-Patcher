...
Rename GameName.exe → GameNameReal.exe
Rename GameName_Data\ → GameNameReal_Data\
Compile and place a wrapper GameName.exe in its place
...
ActionDescriptionLoadLoad a saved profile into the fieldsSaveSave current fields as a named profileDeleteRemove a profile
...
Tested With
GameArgumentsSubnautica: Below Zero-vrmode openvrSubnautica (original)-vrmode openvr
Works with any Unity game that accepts command-line VR arguments.

Troubleshooting
csc.exe not found
Make sure .NET Framework 4.x is installed. It comes pre-installed on Windows 10/11 but can be downloaded from Microsoft.
